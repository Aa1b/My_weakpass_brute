...
"""
Hadoop Storm 未授权访问检测工具

使用方法:
    单个目标:
        python brute_storm.py -t 192.168.1.1
    批量检测:
        python brute_storm.py -a ip.txt
    指定端口:
        python brute_storm.py -t 192.168.1.1 -p 8080
    显示详细信息:
        python brute_storm.py -a ip.txt -v
...
def brute(ip, port):
    ...
    try:
        # 检查Storm UI
        url = f"http://{ip}:{port}/api/v1/cluster/summary"
        response = requests.get(url, verify=False, timeout=5)
        
        if response.status_code == 200:
            _t = f"[+] {ip}:{port} - 存在Storm UI未授权访问"
            save_result(_t)
            logger.info(_t)
            
            try:
                data = response.json()
                version = data.get('stormVersion', '未知')
                _t = f"[+] {ip}:{port} - Storm版本: {version}"
                save_result(_t)
                logger.info(_t)
                ...
                _t = f"[+] {ip}:{port} - 集群信息: {supervisors}个Supervisor, {slots_used}/{slots_total}个Slot使用中"
                save_result(_t)
                logger.info(_t)
            except:
                pass
            ...
            # 获取Topology列表
            try:
                url = f"http://{ip}:{port}/api/v1/topology/summary"
                response = requests.get(url, verify=False, timeout=5)
                if response.status_code == 200:
                    data = response.json()
                    topologies = data.get('topologies', [])
                    _t = f"[+] {ip}:{port} - 发现 {len(topologies)} 个Topology"
                    save_result(_t)
                    logger.info(_t)
                    
                    # 显示每个Topology的信息
                    for topo in topologies:
                        name = topo.get('name', '未知')
                        status = topo.get('status', '未知')
                        uptime = topo.get('uptimeSeconds', 0)
                        _t = f"[+] {ip}:{port} - Topology: {name}, 状态: {status}, 运行时间: {uptime}秒"
                        save_result(_t)
                        logger.info(_t)
            except:
                pass
