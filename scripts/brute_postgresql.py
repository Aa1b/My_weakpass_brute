...
"""
PostgreSQL 未授权访问检测工具

使用方法:
    单个目标:
        python brute_postgresql.py -t 192.168.1.1
    批量检测:
        python brute_postgresql.py -a ip.txt
    指定端口:
        python brute_postgresql.py -t 192.168.1.1 -p 5432
    显示详细信息:
        python brute_postgresql.py -a ip.txt -v
...
def brute(ip, port):
    ...
    try:
        ...
        if conn:
            _t = f"[+] {ip}:{port} - 存在PostgreSQL未授权访问"
            save_result(_t)
            logger.info(_t)
            ...
            try:
                ...
                _t = f"[+] {ip}:{port} - PostgreSQL版本: {version}"
                save_result(_t)
                logger.info(_t)
                ...
                _t = f"[+] {ip}:{port} - 发现 {len(databases)} 个数据库"
                save_result(_t)
                logger.info(_t)
                
                for db in databases:
                    db_name = db[0]
                    _t = f"[+] {ip}:{port} - 数据库: {db_name}"
                    save_result(_t)
                    logger.info(_t)
                    ...
                    try:
                        ...
                        if tables:
                            _t = f"[+] {ip}:{port} - 数据库 {db_name} 中发现 {len(tables)} 个表"
                            save_result(_t)
                            logger.info(_t)
                            
                            for schema, table in tables:
                                _t = f"[+] {ip}:{port} - 数据库 {db_name} 表: {schema}.{table}"
                                save_result(_t)
                                logger.info(_t)
                ...
                logger.debug(f"[-] {ip}:{port} - 获取数据库信息失败: {str(e)}")
            finally:
                conn.close()
...
def main():
    ...
    parser.add_option("-p", dest="port", default="5432", help="target port (default: 5432)")
    ...
    try:
        if options.host:
            ...
            # 2. 端口扫描
            file_name = f"{port}_postgresql.txt"
            logger.info("[*] 开始端口扫描")
