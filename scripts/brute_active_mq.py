...
使用方法:
    单个目标:
        python brute_active_mq.py -t 192.168.1.1
    批量检测:
        python brute_active_mq.py -a ip.txt
    指定端口:
        python brute_active_mq.py -t 192.168.1.1 -p 8161
    显示详细信息:
        python brute_active_mq.py -a ip.txt -v
...
def brute(ip, port):
    ...
    try:
        ...
        if response.status_code == 200 and "ActiveMQ" in response.text:
            _t = f"[+] {ip}:{port} - 存在ActiveMQ未授权访问"
            save_result(_t)
            logger.info(_t)
            
            # 检测点2: 获取版本信息
            version = re.search(r'ActiveMQ ([^<]+)', response.text)
            if version:
                _t = f"[*] {ip}:{port} - ActiveMQ版本: {version.group(1)}"
                save_result(_t)
                logger.info(_t)
            
            # 检测点3: 访问队列管理
            url = f"http://{ip}:{port}/admin/queues.jsp"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200 and "Queue Name" in response.text:
                _t = f"[*] {ip}:{port} - 队列管理页面可访问"
                save_result(_t)
                logger.info(_t)
                
                # 解析队列数量
                queues = len(re.findall(r'<tr class="[^"]*">\s*<td><a[^>]*>([^<]+)</a>', response.text))
                if queues > 0:
                    _t = f"[*] {ip}:{port} - 发现 {queues} 个队列"
                    save_result(_t)
                    logger.info(_t)
            
            # 检测点4: 访问主题管理
            url = f"http://{ip}:{port}/admin/topics.jsp"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200 and "Topic Name" in response.text:
                _t = f"[*] {ip}:{port} - 主题管理页面可访问"
                save_result(_t)
                logger.info(_t)
                
                # 解析主题数量
                topics = len(re.findall(r'<tr class="[^"]*">\s*<td><a[^>]*>([^<]+)</a>', response.text))
                if topics > 0:
                    _t = f"[*] {ip}:{port} - 发现 {topics} 个主题"
                    save_result(_t)
                    logger.info(_t)
            
            # 检测点5: 访问连接管理
            url = f"http://{ip}:{port}/admin/connections.jsp"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200 and "Connection ID" in response.text:
                _t = f"[*] {ip}:{port} - 连接管理页面可访问"
                save_result(_t)
                logger.info(_t)
                
                # 解析连接数量
                connections = len(re.findall(r'<tr class="[^"]*">\s*<td><a[^>]*>([^<]+)</a>', response.text))
                if connections > 0:
                    _t = f"[*] {ip}:{port} - 发现 {connections} 个活动连接"
                    save_result(_t)
                    logger.info(_t)
...
def main():
    ...
    parser.add_option("-p", dest="port", default="8161", help="target port (default: 8161)")
    ...
    try:
        if options.host:
            ...
            # 2. 端口扫描
            file_name = f"{port}_active_mq.txt"
            logger.info("[*] 开始端口扫描")
