...
使用方法:
    单个目标:
        python brute_mysql.py -t 192.168.1.1 -u users.txt -P pass.txt
    批量检测:
        python brute_mysql.py -a ip.txt -u users.txt -P pass.txt
    指定端口:
        python brute_mysql.py -t 192.168.1.1 -p 3307 -u users.txt -P pass.txt
    显示详细信息:
        python brute_mysql.py -a ip.txt -u users.txt -P pass.txt -v
...
def main():
	...
	parser.add_option("-p", dest="port", default="3306", help="target port (default: 3306)")
