...
使用方法:
    单个目标:
        python brute_redis.py -t 192.168.1.1
    批量检测:
        python brute_redis.py -a ip.txt
    指定端口:
        python brute_redis.py -t 192.168.1.1 -p 6379
    显示详细信息:
        python brute_redis.py -a ip.txt -v
...
def brute(ip, port):
    ...
    try:
        # 使用redis-cli命令进行检测
        cmd = f"redis-cli -h {ip} -p {port} info"
        ...
        if process.returncode == 0 and "redis_version" in stdout:
            _t = f"[+] {ip}:{port} - 存在Redis未授权访问"
            save_result(_t)
            logger.info(_t)
            ...
            if "redis_version" in stdout:
                ...
                if version:
                    _t = f"[*] {ip}:{port} - Redis版本: {version.group(1)}"
                    save_result(_t)
                    logger.info(_t)
            return True
...
def main():
    ...
    parser.add_option("-p", dest="port", default="6379", help="target port (default: 6379)")
