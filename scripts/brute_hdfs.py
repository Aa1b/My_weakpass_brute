...
"""
Hadoop HDFS 未授权访问检测工具

使用方法:
    单个目标:
        python brute_hdfs.py -t 192.168.1.1
    批量检测:
        python brute_hdfs.py -a ip.txt
    指定端口:
        python brute_hdfs.py -t 192.168.1.1 -p 50070
    显示详细信息:
        python brute_hdfs.py -a ip.txt -v
...
支持检测的端口:
    - 50070: Hadoop 2.x NameNode Web UI端口
    - 9870: Hadoop 3.x NameNode Web UI端口
"""
        ...
        if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    for bean in data.get('beans', []):
                        if bean.get('name') == 'Hadoop:service=NameNode,name=NameNodeInfo':
                            version = bean.get('SoftwareVersion', '未知')
                            _t = f"[+] {ip}:{port} - Hadoop版本: {version}"
                            save_result(_t)
                            logger.info(_t)
                            
                            live_nodes = bean.get('LiveNodes', '{}')
                            dead_nodes = bean.get('DeadNodes', '{}')
                            try:
                                live_nodes = json.loads(live_nodes)
                                dead_nodes = json.loads(dead_nodes)
                                _t = f"[+] {ip}:{port} - 集群状态: {len(live_nodes)}个活跃节点, {len(dead_nodes)}个死亡节点"
                                save_result(_t)
                                logger.info(_t)
                            except:
                                pass
                            ...
                            total_files = bean.get('TotalFiles', 0)
                            _t = f"[+] {ip}:{port} - 文件系统信息: {total_files}个文件, {total_blocks}个数据块"
                            save_result(_t)
                            logger.info(_t)
                            break
            except:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    _t = f"[+] {ip}:{port} - 根目录下发现 {len(files)} 个文件/目录"
                    save_result(_t)
                    logger.info(_t)
                    
                    for file in files:
                        name = file.get('pathSuffix', '')
                        type = file.get('type', '')
                        size = file.get('length', 0)
                        _t = f"[+] {ip}:{port} - {type}: {name}, 大小: {size}字节"
                        save_result(_t)
                        logger.info(_t)
            except:
                pass
...
def main():
    ...
    parser.add_option("-p", dest="port", default="50070", help="target port (default: 50070)")
