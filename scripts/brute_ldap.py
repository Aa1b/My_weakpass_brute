...
使用方法:
    单个目标:
        python brute_ldap.py -t 192.168.1.1
    批量检测:
        python brute_ldap.py -a ip.txt
    指定端口:
        python brute_ldap.py -t 192.168.1.1 -p 389
    显示详细信息:
        python brute_ldap.py -a ip.txt -v
...
def brute(ip, port):
    ...
    try:
        ...
        if conn.bind():
            # 尝试获取根DSE信息
            if server.info:
                _t = f"[+] {ip}:{port} - 存在LDAP未授权访问"
                save_result(_t)
                logger.info(_t)
                
                # 获取更多信息
                if server.info.naming_contexts:
                    _t = f"[*] {ip}:{port} - 命名上下文: {', '.join(server.info.naming_contexts)}"
                    save_result(_t)
                    logger.info(_t)
            conn.unbind()
            return True
...
def main():
    ...
    parser.add_option("-p", dest="port", default="389", help="target port (default: 389)")
