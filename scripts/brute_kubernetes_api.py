...
"""
Kubernetes Api Server 未授权访问检测工具

使用方法:
    单个目标:
        python brute_kubernetes_api.py -t 192.168.1.1
    批量检测:
        python brute_kubernetes_api.py -a ip.txt
    指定端口:
        python brute_kubernetes_api.py -t 192.168.1.1 -p 8080
    显示详细信息:
        python brute_kubernetes_api.py -a ip.txt -v
...
支持检测的端口:
    - 8080: Kubernetes Api Server默认端口
    - 10250: Kubelet默认端口
"""
...
def brute(ip, port):
    ...
    try:
        ...
        if response.status_code == 200:
            _t = f"[+] {ip}:{port} - 存在Kubernetes API Server未授权访问"
            save_result(_t)
            logger.info(_t)
            
            # 检测点2: 获取节点信息
            url = f"http://{ip}:{port}/api/v1/nodes"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200:
                ...
                for node in nodes:
                    ...
                    node_status = node.get('status', {}).get('conditions', [{}])[-1].get('type', '')
                    _t = f"[*] {ip}:{port} - 节点: {node_name}, 状态: {node_status}"
                    save_result(_t)
                    logger.info(_t)
            
            # 检测点3: 获取命名空间信息
            url = f"http://{ip}:{port}/api/v1/namespaces"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200:
                namespaces = response.json().get('items', [])
                _t = f"[*] {ip}:{port} - 发现 {len(namespaces)} 个命名空间"
                save_result(_t)
                logger.info(_t)
            
            # 检测点4: 获取Pod信息
            url = f"http://{ip}:{port}/api/v1/pods"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200:
                pods = response.json().get('items', [])
                _t = f"[*] {ip}:{port} - 发现 {len(pods)} 个Pod"
                save_result(_t)
                logger.info(_t)
            
            # 检测点5: 获取Service信息
            url = f"http://{ip}:{port}/api/v1/services"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200:
                services = response.json().get('items', [])
                _t = f"[*] {ip}:{port} - 发现 {len(services)} 个Service"
                save_result(_t)
                logger.info(_t)
...
def main():
    ...
    parser.add_option("-p", dest="port", default="8080", help="target port (default: 8080)")
    ...
    try:
        if options.host:
            ...
            # 2. 端口扫描
            file_name = f"{port}_kubernetes_api.txt"
            logger.info("[*] 开始端口扫描")
