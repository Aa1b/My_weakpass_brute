...
使用方法:
    单个目标:
        python brute_jenkins.py -t 192.168.1.1
    批量检测:
        python brute_jenkins.py -a ip.txt
    指定端口:
        python brute_jenkins.py -t 192.168.1.1 -p 8080
    显示详细信息:
        python brute_jenkins.py -a ip.txt -v
...
def brute(ip, port):
    ...
    try:
        ...
        if response.status_code == 200 and "Jenkins" in response.text:
            _t = f"[+] {ip}:{port} - 存在Jenkins未授权访问"
            save_result(_t)
            logger.info(_t)
            
            # 检测点2: 获取Jenkins版本
            version = re.search(r'Jenkins ver\. ([^<]+)', response.text)
            if version:
                _t = f"[*] {ip}:{port} - Jenkins版本: {version.group(1)}"
                save_result(_t)
                logger.info(_t)
            
            # 检测点3: 访问脚本命令接口
            url = f"http://{ip}:{port}/script"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200 and "Groovy" in response.text:
                _t = f"[*] {ip}:{port} - 脚本命令接口可访问"
                save_result(_t)
                logger.info(_t)
            
            # 检测点4: 访问用户列表
            url = f"http://{ip}:{port}/asynchPeople/"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200 and "Jenkins User Id" in response.text:
                _t = f"[*] {ip}:{port} - 用户列表可访问"
                save_result(_t)
                logger.info(_t)
            
            # 检测点5: 访问系统信息
            url = f"http://{ip}:{port}/systemInfo"
            response = requests.get(url, headers=headers, timeout=5, verify=False)
            if response.status_code == 200 and "System Properties" in response.text:
                _t = f"[*] {ip}:{port} - 系统信息可访问"
                save_result(_t)
                logger.info(_t)
